...
/// <summary>
/// Scrapper for Amapá (ALEAP).
/// Attempts static HTML parse from the ALEAP deputy list page.
/// </summary>
public class AleapScrapperService : HtmlScrapperBase, IStateDeputyScrapperService
{
    public string StateCode => "AP";
    public string AssemblyName => "ALEAP";

    private static readonly string[] CandidateUrls =
    [
        "https://www.aleap.ap.leg.br/deputados",
        "https://www.aleap.ap.leg.br/parlamentar",
        "https://www.aleap.ap.leg.br/deputados/lista",
        "https://aleap.ap.leg.br/deputados"
    ];

    public AleapScrapperService(IHttpClientFactory httpClientFactory, ILogger<AleapScrapperService> logger)
    ...
    public async Task<List<StateDeputyData>> FetchDeputiesAsync()
    {
        foreach (var url in CandidateUrls)
        {
            try
            {
                Logger.LogInformation("Trying ALEAP URL: {Url}", url);
                ...
                if (deputies.Count > 0)
                {
                    Logger.LogInformation("Successfully fetched {Count} deputies from ALEAP via {Url}", deputies.Count, url);
                    return deputies;
                }

                Logger.LogWarning("No deputies parsed from ALEAP at {Url}", url);
                LogRawHtml(doc, AssemblyName);
            }
            catch (Exception ex)
            {
                Logger.LogWarning(ex, "ALEAP URL {Url} failed, trying next", url);
            }
        }

        Logger.LogWarning("Could not fetch deputies from ALEAP (AP)");
        return [];
    }
