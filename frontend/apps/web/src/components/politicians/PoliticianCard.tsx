...
export function PoliticianCard({ politician: p }: Props) {
  ...
  return (
    <Link
      ...
      className="group bg-white rounded-xl border hover:border-primary-300 hover:shadow-md transition-all flex flex-col overflow-hidden"
    >
      {/* Photo */}
      <div className="relative h-48 bg-slate-50 flex items-center justify-center overflow-hidden">
        {showPhoto ? (
          ...
          <div className="flex flex-col items-center justify-center w-full h-full bg-gradient-to-br from-slate-50 to-slate-100">
            <div className="w-20 h-20 rounded-full bg-primary-100 border-2 border-primary-200 flex items-center justify-center shadow-sm">
      ...
      <div className="p-4 flex-1 flex flex-col gap-1">
        <h3 className="font-semibold text-slate-900 text-sm leading-tight group-hover:text-primary-700 transition-colors line-clamp-2">
          {p.fullName}
        </h3>
        <p className="text-xs text-slate-500">{positionLabel(p.politicalPosition)}</p>
