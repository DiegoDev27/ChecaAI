...
const ROLE_COLOR: Record<string, string> = {
  Presidente:      'bg-primary-100 text-primary-800',
  'Vice-Presidente': 'bg-primary-100 text-primary-800',
  Titular:         'bg-green-100 text-green-800',
  Suplente:        'bg-yellow-100 text-yellow-800',
};

function roleColor(role: string) {
  return ROLE_COLOR[role] ?? 'bg-slate-100 text-slate-700';
}
  ...
  if (committees.length === 0) {
    return (
      <div className="bg-white rounded-xl border p-10 text-center">
        ...
        <p className="text-slate-500 font-medium text-sm mb-1">Nenhuma comissão registrada</p>
      ...
      <div className="bg-white rounded-xl border p-4 flex items-center gap-6 flex-wrap">
        ...
        {Object.entries(grouped).map(([type, items]) => (
          <div key={type} className="text-sm text-slate-500">
            <span className="font-medium text-slate-800">{items.length}</span> {type.toLowerCase()}
          </div>
        ))}
      </div>
      ...
      {sortedGroups.map(([type, items]) => (
        <div key={type} className="bg-white rounded-xl border overflow-hidden">
          <div className="px-5 py-3 bg-slate-50 border-b">
            <h3 className="text-sm font-semibold text-slate-700">{type}</h3>
          </div>
              ...
              <div key={c.committeeId} className="flex items-center gap-4 px-5 py-3.5">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-800 leading-snug">
                    {c.committeeName}
                    {c.acronym && (
                      <span className="ml-1.5 text-xs text-slate-400">({c.acronym})</span>
                    )}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">{c.chamber}</div>
                </div>
