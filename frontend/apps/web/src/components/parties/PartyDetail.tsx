...
export function PartyDetail({ acronym }: Props) {
  ...
  if (partyError || !party) {
    return (
      <div className="text-center py-12 space-y-4">
        ...
        <Link href="/partidos" className="text-primary-600 hover:underline text-sm">
  ...
  return (
    <div className="space-y-6">
      ...
      <Link
        href="/partidos"
        className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-primary-700 transition-colors"
      >
      ...
      <div className="bg-white rounded-2xl border shadow-sm p-6">
        <div className="flex flex-col sm:flex-row items-start gap-5">
          ...
          <div className="w-20 h-20 rounded-xl border-2 border-slate-100 flex items-center justify-center flex-shrink-0 bg-white overflow-hidden">
            {!logoError ? (
              <Image
              ...
              <div className="w-full h-full bg-primary-100 border-2 border-primary-200 rounded-xl flex items-center justify-center">
                <span className="text-xl font-bold text-primary-700 px-1 text-center leading-tight">
                  {party.acronym}
                </span>
              </div>
            )}
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold text-slate-900 leading-tight">{party.fullName}</h1>
            <div className="flex flex-wrap items-center gap-3 mt-2">
              {party.number && (
                <span className="text-sm bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full font-mono">
                  Nº {party.number}
                </span>
              )}
              <span className={cn(
          ...
          <div className="text-center sm:text-right flex-shrink-0">
            ...
            <div className="text-sm text-slate-500 mt-0.5 flex items-center gap-1 justify-center sm:justify-end">
        ...
        {party.president && (
          <div className="mt-4 pt-4 border-t">
            <span className="text-sm text-slate-500">Presidente do partido:</span>{' '}
            <span className="text-sm font-semibold text-slate-800">{party.president}</span>
          </div>
        )}
      </div>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="p-2 rounded-lg border text-sm font-medium disabled:opacity-40 hover:bg-slate-50 transition-colors"
                  >
...
function MemberCard({ politician }: { politician: PoliticianListItem }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 bg-white rounded-xl border hover:border-primary-300 hover:shadow-sm transition-all p-3 group"
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-slate-800 truncate group-hover:text-primary-700 transition-colors leading-tight">
          {politician.fullName}
        </div>
