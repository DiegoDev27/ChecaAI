...
function ResultBadge({ result }: { result: string }) {
  ...
  return (
    <span className={cn(
      ...
      approved ? 'bg-green-100 text-green-800' : rejected ? 'bg-red-100 text-red-800' : 'bg-slate-100 text-slate-700',
    )}>
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="text-sm text-slate-500 flex items-center gap-1 flex-wrap">
        <Link href="/proposicoes" className="hover:text-primary-600">Proposições</Link>
      ...
      <div className="bg-white rounded-xl border p-6">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <span className="text-sm font-bold text-primary-700 bg-primary-50 border border-primary-200 px-3 py-1 rounded-lg">
            {p.type} {p.number && `${p.number}/`}{p.year}
          </span>
          <span className={cn(
            'text-xs font-medium px-2.5 py-1 rounded-full border',
            STATUS_STYLE[p.status] ?? 'bg-slate-100 text-slate-700 border-slate-200',
          )}>
            {p.status}
          </span>
        </div>

        <h1 className="text-xl font-bold text-slate-900 leading-snug mb-4">{p.title}</h1>

        {p.summary && (
          <div className="bg-slate-50 rounded-lg p-4 border text-sm text-slate-700 leading-relaxed mb-4">
            <p className="font-medium text-xs text-slate-500 mb-2 uppercase tracking-wide">Ementa</p>
            <p>{p.summary}</p>
          </div>
        )}
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 text-sm">
          {p.author && (
            <div className="flex items-center gap-2 text-slate-600">
              ...
              <span className="truncate">Autor: <span className="font-medium text-slate-800">{p.author}</span></span>
            </div>
          )}
          <div className="flex items-center gap-2 text-slate-600">
      ...
      {p.votingSessions && p.votingSessions.length > 0 ? (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className="text-xs text-slate-400">{s.chamber}</span>
                      {s.sessionType && (
                        <span className="text-xs text-slate-400">• {s.sessionType}</span>
                      )}
                    </div>
                    <p className="text-sm text-slate-800 leading-snug group-hover:text-primary-700 line-clamp-2">
                      {s.description}
                    </p>
                  ...
                  <ChevronRight className="h-4 w-4 text-slate-300 flex-shrink-0 mt-1 group-hover:text-primary-500 transition-colors" />
                </Link>
              );
            })}
          </div>
        ...
        <div className="bg-white rounded-xl border p-6 text-center text-slate-400 text-sm">
          <Vote className="h-10 w-10 mx-auto mb-2 text-slate-200" />
